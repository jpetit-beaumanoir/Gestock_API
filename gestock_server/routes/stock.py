...
async def get_stock_caja(
    ...
    try:
        ...
        logging.info(f"{request.state.user} HA CONSULTAT EL STOCK DE LA CAIXA {data.caja} DEL PALET {data.palet} EN EL MAGATZEM {data.almacen} ({request.client.host})")

        return result

    except ConnectionError:
        logging.error(
            f"ERROR DE BD OBTENINT STOCK DE LA CAIXA {data.caja} DEL PALET {data.palet} EN EL MAGATZEM {data.almacen} ({request.client.host})"
        )
...
async def export_stock(
    ...
    try:
        ...
        logging.info(f"{request.state.user} HA EXPORTAT STOCK DEL MAGATZEM {data.almacen} ({request.client.host})")

        return result

    except ConnectionError:
        logging.error(
            f"ERROR DE BD EXPORTANT STOCK DEL MAGATZEM {data.almacen} ({request.client.host})"
        )
